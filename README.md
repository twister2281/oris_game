...
#### На одном компьютере (для тестирования):
```bash
java -cp bin game.Main
```
1. Запустите первый экземпляр и выберите **Server**
2. Запустите второй экземпляр и выберите **Client**
3. В поле адреса введите `localhost` или оставьте пустым

#### На двух компьютерах в сети:

**На компьютере 1 (Сервер):**
1. Узнайте IP-адрес компьютера:
   - **Windows**: откройте командную строку и выполните `ipconfig`, найдите IPv4-адрес
   - **macOS/Linux**: откройте терминал и выполните `ifconfig` или `ip addr`, найдите IP-адрес (обычно начинается с 192.168.x.x или 10.x.x.x)
2. Запустите игру: `java -cp bin game.Main`
3. Выберите **Server**
4. Дождитесь сообщения "Server started. Waiting for client..."

**На компьютере 2 (Клиент):**
1. Убедитесь, что оба компьютера в одной сети (Wi-Fi или локальная сеть)
2. Запустите игру: `java -cp bin game.Main`
3. Выберите **Client**
4. В поле "Enter server address" введите IP-адрес компьютера 1 (например: `192.168.1.100`)
5. Нажмите OK

**Важно:**
- Убедитесь, что порт **12345** не заблокирован файрволом на сервере
- Если подключение не работает, проверьте файрвол и разрешите входящие соединения на порт 12345
- Оба компьютера должны быть в одной локальной сети
